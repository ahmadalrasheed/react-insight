...
const checkLayoutShift = () => {
  // This is a simplified version - in a real implementation, you'd use the Layout Instability API
  // or a more sophisticated approach to detect layout shifts
  const elements = document.querySelectorAll('*');
  let potentialLayoutShifts = [];
...
const checkDOMSize = () => {
  // Use a direct approach to count all DOM elements
  // Count elements in the document body
  const bodyElements = document?.body?.getElementsByTagName('*')?.length || 0;
  
  // Count elements in the document head
  const headElements = document?.head?.getElementsByTagName('*')?.length || 0;
  
  // Total elements is the sum of body and head elements
...
// 1. JavaScript Heap Size (Memory Usage)
...
// 2. Number of Active Intervals/Timeouts
const checkActiveTimers = () => {
  ...
  return allTimers?.length || 0; // best-effort only; limited accuracy
};
...
// 4. Interaction Responsiveness (First Input Delay Approximation)
...
// 5. Check for pending network requests
const checkPendingNetworkRequests = () => {
  // This is a simplified check - in a real implementation, you'd use the Performance API
  // or a more sophisticated approach to detect pending network requests
  
  // Check for fetch requests that might be pending
  const pendingFetches = window?.performance?.getEntriesByType('resource')
    ?.filter(entry => entry?.duration > 0 && entry?.responseEnd === 0) || [];
...
const checkPerformance = async () => {
  ...
  // First, determine the maximum possible score based on DOM size
  let maxScore = 100;
  
  // Set maximum score cap based on DOM size ranges
  if (domSize?.totalElements > 3000) {
    maxScore = 50; // Very large DOM (3000+ elements)
  } else if (domSize?.totalElements > 2000) {
    maxScore = 65; // Large DOM (2000-3000 elements)
  } else if (domSize?.totalElements > 1000) {
    maxScore = 75; // Medium-large DOM (1000-2000 elements)
  }
  
  // Start with the maximum score
  let score = maxScore;
  
  // Apply penalties for other issues, but don't go below 0
  if (domSize?.depth > 10) {
    score -= Math.min(10, (domSize?.depth - 10));
  }
  
  if (imagesAlt?.withoutAlt > 0) {
    score -= Math.min(15, imagesAlt?.withoutAlt * 2);
  }
  
  if (layoutShift?.total > 0) {
    score -= Math.min(10, layoutShift?.total);
  }
  
  // New penalties for memory usage
  if (memoryUsage && memoryUsage?.usedMB > 100) {
    score -= Math.min(15, (memoryUsage?.usedMB - 100) / 10);
  }
  
  // Penalties for active timers
  if (activeTimers > 10) {
    score -= Math.min(10, (activeTimers - 10));
  }
  
  // Penalties for event listeners
  if (eventListeners?.totalListeners > 500) {
    score -= Math.min(15, (eventListeners?.totalListeners - 500) / 50);
  }
  
  // Penalties for interaction delay
  if (interactionDelay > 100) {
    score -= Math.min(20, (interactionDelay - 100) / 10);
  }
  
  // Penalties for pending network requests
  if (pendingRequests?.total > 5) {
    score -= Math.min(10, (pendingRequests?.total - 5));
  }
...
const PerformanceMonitor = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Fix the unused expression by properly handling the document event listener
    const doc = document;
    if (doc) {
